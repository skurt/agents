...
Read the `version` field from #file:../plugin.json and display the following message to the user, substituting `{version}` with the value read:

> Welcome to the automated Partner Quality Audit agent — version {version}.
